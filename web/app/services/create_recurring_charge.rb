  ...
  def create_charge(plan)
    begin
      ...
      variables = {
        ...
        name: plan.name,
        returnUrl: "https://#{@shopify_domain}/admin/apps/#{ENV.fetch('APP_NAME')}?shop=#{@shopify_domain}",
      ...
      raise CreateRecurringCharge::InvalidRequest, response.body.dig('errors', 0, 'message') and return unless response.body['errors'].nil?

      return response.body.dig('data', 'appSubscriptionCreate', 'confirmationUrl')
